...
=============================================================
 ...
 Version 10.0.0 — TIMESFM INTÉGRÉ (v9 + TimesFM 2.5)
 Basé sur v9.0.0 — 6 bug fixes + TimesFM Validator
 NOUVEAUTÉ : Valideur TimesFM (filtre de direction avant exécution)
=============================================================
"""
...
TIMESFM_TIMEFRAME       = os.getenv("TIMESFM_TIMEFRAME", "M5")
...
TIMESFM_SYMBOL          = os.getenv("TIMESFM_SYMBOL", "XAUUSDm")
...
class TimesFMValidator:
    ...
    Modèle : TimesFM 2.5 (200M paramètres, google/timesfm-2.5-200m-pytorch)
    """
    ...
    def validate(self, signal_direction: str) -> dict:
        ...
        try:
            ...
            log.info(
                ...
                f"Move={move_pips:.1f}pips Conf={confidence:.2f} → "
                f"{'✅ VALID' if valid else '❌ REJETÉ'}"
            )
            if not valid:
...
async def main():
    ...
    try:
        ...
        mode = "🧪 DEMO" if DEMO_MODE else "💰 LIVE"
        log.info("=" * 55)
        log.info(f" TRADINGBOT V10.0.0 — TIMESFM INTÉGRÉ")
