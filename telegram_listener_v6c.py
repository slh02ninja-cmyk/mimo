...
=============================================================
 ...
 Version 4.8.6 — GAIN FIXE + BE OPTIMISÉ + TTL VARIABLE
 CORRECTIONS :
 - Cache _resolve_order TTL paramétrable (CACHE_TTL_SECONDS)
 - BE déclenché uniquement sur le MARKET
 - Objectif basé sur _expected_positions
 - BE appliqué aux positions remplies après BE
=============================================================
"""
...
# === CACHE TTL (paramétrable) ===
CACHE_TTL_SECONDS = int(os.getenv("CACHE_TTL_SECONDS", "1"))  # ★★★ NOUVEAU ★★★
...
class TradeManager:
    def __init__(self, bridge: MT5Bridge, tracker=None):
        ...
        # ★★★ BUG #8 : cache pour _resolve_order() avec TTL paramétrable ★★★
        self._order_cache = {}          # {order_ticket: position_ticket ou None}
        self._cache_ttl = CACHE_TTL_SECONDS   # ★★★ Utilisation de la variable d'env ★★★
        self._cache_timestamps = {}     # {order_ticket: datetime}
        log.info(f"[CACHE] TTL configuré à {self._cache_ttl} secondes")
    ...
    def _check_pnl_trigger(self, entry: dict) -> bool:
        """Vérifie si le P&L de la position MARKET atteint PNL_TRIGGER_USD."""
        for t in entry.get("tickets", []):
            if t.get("be_active"):
                continue
    ...
    def _apply_be_on_open_positions(self, entry: dict, action: str):
        ...
        # Positions ouvertes détectées (pour appliquer le BE)
        open_tickets = [t for t in entry.get("tickets", []) if self._get_pos(t["ticket"])]
        open_count = len(open_tickets)
        if open_count == 0:
            log.warning(f"Aucune position ouverte au moment du BE pour le signal {entry.get('_signal_id', '?')}")
            return

        # ★★★ Utiliser _expected_positions pour l'objectif ★★★
        expected = entry.get("_expected_positions", 1)
        gain_per_position = self._get_gain_per_position(entry)
        target_gain = gain_per_position * expected

        # Calcul du BE
        if open_count == 1:
            # Une seule position ouverte (le MARKET) : BE = son entry
            t = open_tickets[0]
            ...
            if pos:
                ...
                log.info(f"🔒 BE 1 POSITION (mais {expected} attendues) : SL = {be_price:.2f} (P&L = 0$)")
                if self.bridge.modify_sl(t["ticket"], be_price, f"[BE 1POS @{be_price}]"):
                    t["be_active"] = True
                    t["be_sl"] = be_price
                    log.info(f"  ✓ BE sur #{t['ticket']} → SL={be_price}")
            # Stocker pour les futures positions
            entry["_be_price"] = be_price
            entry["_be_market_entry"] = entry_price

        elif open_count == 2:
            # Deux positions ouvertes (si la LIMIT a été détectée) : BE = médian
        ...
        log.info(f"   Signal {entry.get('_signal_id', '?')} : Objectif = {target_gain:.2f}$ ({gain_per_position:.2f}$ × {expected} positions attendues)")
        ...
        send_alert_sync(
            ...
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Positions détectées: {open_count} ({be_type})\n"
            f"Objectif: {target_gain:.2f}$ ({expected} positions attendues)\n"
            f"SL: {be_price:.2f}\n"
    ...
    def _check_pending_only_expiry(self, entry: dict, symbol: str, action: str):
        ...
        if action == "BUY" and current >= tp_trigger:
            ...
            triggered = True
        if triggered:
            log.info(f"⚠️ TP_TRIGGER ({tp_trigger:.2f}) atteint sans position ouverte → annulation des ordres pending")
            self._cancel_pending_orders_for_entry(entry)
            signal = entry.get("signal", {})
            canal = signal.get("source_channel", "Inconnu")
            send_alert_sync(
    ...
    # ★★★ BUG #3 : suppression de deal.order == ticket ★★★
    def _get_last_pnl(self, ticket: int, symbol: str) -> float:
        ...
        if deals is None:
            return 0.0
        for deal in reversed(deals):
            if deal.position_id == ticket:   # ✅ uniquement position_id
        ...
        for entry in entries_snapshot:
            ...
            for o in entry.get("orders", []):
                ...
                if pos:
                    ...
                    # ★★★ NOUVEAU : Si BE déjà activé, appliquer le BE sur la nouvelle position ★★★
                    if entry.get("_be_activated", False) and not tk.get("be_active", False):
                        be_price = entry.get("_be_price")
                        if be_price is not None:
                            if self.bridge.modify_sl(pos.ticket, be_price, f"[BE LATE @{be_price}]"):
                                tk["be_active"] = True
                                tk["be_sl"] = be_price
                                log.info(f"  ✓ BE appliqué sur nouvelle position #{pos.ticket} → SL={be_price}")
                            else:
                                log.warning(f"  ✗ Échec du BE sur nouvelle position #{pos.ticket}")
                        else:
                            log.warning(f"  ⚠️ BE déjà activé mais _be_price non défini pour #{pos.ticket}")
        ...
        if order_ticket in self._order_cache:
            ...
            if (now - cache_time).total_seconds() < self._cache_ttl:
                ...
                if pos:
                    ...
                    # position fermée, supprimer du cache
                    del self._order_cache[order_ticket]
                    del self._cache_timestamps[order_ticket]
                    return None
        
        # Cache invalide → requête MT5 (1 jour + filtre symbole)
...
# =============================================================
# QUICK ALERT (avec bug #4 et #6 corrigés)
# =============================================================
...
def execute_quick_alert(signal: dict, bridge: MT5Bridge, manager: TradeManager,
    ...
    if key in quick_alerts and quick_alerts[key]:
        ...
        if existing.get("is_limit", False):
            ...
            if order:
                # ★★★ BUG #4 : utiliser modify_pending_order() ★★★
                bridge.modify_pending_order(existing_ticket, sl, default_tp, "[QA-UPDATE-SL-TP]")
                log.info(f"  ✓ SL/TP de l'ordre pending #{existing_ticket} mis à jour → SL={sl}, TP={default_tp}")
...
async def main():
    ...
    mode = "🧪 DEMO" if DEMO_MODE else "💰 LIVE"
    log.info("=" * 55)
    log.info(f" TRADINGBOT V4.8.6 — GAIN FIXE + BE OPTIMISÉ + TTL VARIABLE")
    ...
    log.info(f" TP_TRIGGER pending : utilisé pour annuler les ordres sans position ouverte")
