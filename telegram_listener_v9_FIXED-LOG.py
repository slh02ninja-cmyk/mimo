        ...
        for entry in entries_snapshot:
            ...
            for t in entry.get("tickets", []):
                ...
                if pos is None and not t.get("_reported"):
                    ...
                    log.info(f">>>>> P&L QUOTIDIEN : {daily_pnl_now:.2f} $")
...
def execute_signal(signal: dict, bridge: MT5Bridge, manager, tracker):
    ...
    all_tps = signal["tps"]
    if not all_tps:
        log.info(f"CH{ch_num} | REFUSÉ | AUCUN TP TROUVÉ")
    ...
    if not DEMO_MODE and check_conflict(signal, bridge, manager):
        log.info(f"CH{ch_num} | REFUSÉ | CONFLIT DE POSITION")
        return

    sym_info = bridge._sym(symbol)
    if sym_info is None:
        log.info(f"CH{ch_num} | REFUSÉ | SYMBOLE INTROUVABLE")
        log.error(f"Signal rejeté — symbole introuvable dans MT5: {symbol}")
        return

    current = bridge.current_price(sym_info.name, action)
    if current is None:
        log.info(f"CH{ch_num} | REFUSÉ | PRIX INDISPONIBLE")
    ...
    if not SignalParser._validate_sl(action, avg_entry, sl):
        log.info(f"CH{ch_num} | REFUSÉ | SL INVALIDE")
    ...
    if tick and not DEMO_MODE:
        ...
        if spread_pips > MAX_SPREAD_POINTS:
            log.info(f"CH{ch_num} | REFUSÉ | SPREAD TROP LARGE ({spread_pips:.0f} pts)")
    ...
    if total_signals >= MAX_POSITIONS:
        log.info(f"CH{ch_num} | REFUSÉ | MAX SIGNAUX ATTEINT ({total_signals}/{MAX_POSITIONS})")
    ...
    if is_single_price and len(all_tps) >= 1:
        ...
        if scenario == 3:
            log.info(f"CH{ch_num}-PU | REFUSÉ | PRIX HORS ZONE")
        ...
        if not orders and not tickets:
            log.info(f"CH{ch_num}-PU | REFUSÉ | ÉCHEC PLACEMENT ORDRE")
    ...
    if in_zone:
        ...
        if between_zone_tp1:
            ...
            log.info(f"CH{ch_num}-C | REFUSÉ | PRIX ATTEINT TP3")
            log.debug(f"CAS 2 — prix hors zone → ANNULÉ | prix={current} | TP1={tp1} TP2={tp2}")
            return

    if not orders and not tickets:
        log.info(f"CH{ch_num}-C | REFUSÉ | ÉCHEC PLACEMENT ORDRE")
...
async def main():
    ...
    try:
        ...
        if acc_info:
            log.info(f"Balance : {acc_info.balance:.2f}$ | P&L quotidien : {manager._daily_pnl:.2f}$")
        await manager.start()
